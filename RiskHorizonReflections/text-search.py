...
data = getJSONData()
reflections = searchForTerm(getReflections(data), "level 1")
entriesCount = len(reflections)

print "found " + str(entriesCount) + " entries"
for i in range(entriesCount):
    summary = data["submissions"][i]["reflection"][:50]
    path = data["submissions"][i]["path"]
